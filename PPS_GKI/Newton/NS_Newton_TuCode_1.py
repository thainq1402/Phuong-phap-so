...
def Input():
    x_t: list[float] = []
    y_t: list[float] = []
    with open('C:\\Users\\Trung Nguyen\\OneDrive\\Desktop\\20221\\PPS\\17. Noi suy Newton\\NS Newton\\input.txt', 'r+') as f:
...
if __name__ == "__main__":
    ...
    len_x = len(x)
    deg=len_x-1
    print(" da thuc P(n) bậc : "+str(deg) )

    Coef = Bang_Ty_Sai_Phan(x,y,deg) # Bang ty sai phan
    he_so,poly=NS_Newton(x,y,Coef,deg) # Tinh he so cua da thuc noi suy 
    ...
    x_new_1,y_new_1 = Toi_Uu_Loai_1(x,y)

    print(str(x_new_1)+"\n")
    print()
    print(str(y_new_1)+"\n")

    Coef_1 = Bang_Ty_Sai_Phan(x_new_1,y_new_1,deg)
    he_so_1,po_ly_1=NS_Newton(x_new_1,y_new_1,Coef_1,deg)

    print(DataFrame(Coef_1)) # bang ty sai phan sau khi toi uu
    print()
    print(he_so_1) # He so cua da thuc noi suy sau khi toi uu


   

    Xap_xi_1=float(input("Nhap gia tri xap xi : "))
    #Xap_xi_1=math.cos((2*5+1)*math.pi/(2*deg))
    Gia_tri_1=Hoocne(Xap_xi_1,he_so_1)
    print(Gia_tri_1)

    # print("\nToi uu moc noi suy loai 2 \n")

    # x_new_2,y_new_2=Toi_Uu_Loai_2(x,y)
